...
+label+::
  An optional hint providing suggested text that can help clients in 
  rendering / captioning the data element, client-side. Clients MAY
  take the label under consideration or ignore it completely.
+rel+::
...
=== Document Map
Below is a simple `map' of the UBER document format (XML variant). Along with the three elements, there are ten properties (indicated by the `@' character). The +<data>+ element can appear as a child element of +<uber>+ and +<error>+ and may be nested as many times as needed.
