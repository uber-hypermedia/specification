...
The UBER message model is very minimal. The list of elements, properties, and reserved values is rather small in order to keep the process of describing and processing messages as lightweight as possible.

=== Elements
Below is the list of all possible elements in an UBER message:
...
=== Properties
Below is a list of all the possible properties in an UBER message:
...
+url+::
  A resolvable URL associated with this element. The value MAY be a URL Template.
+templated+::
...
There are no UBER-specific link relation values. Document authors SHOULD use registered link relation values that comply the guidance provided in RFC5988 <<rfc5988, [RFC5988]>> and from shared public sources such as (but not limited to):
...
+model+::
  Contains a RFC6570-compliant <<rfc6570,[RFC6570]>> string to be used to construct message bodies. Variables in UBER +model+ strings SHOULD be resolved using the values from +name+ properties, but MAY come from any source available to the client application.
...
If the +templated+ property of a +data+ element is set to +true+ the +url+ property SHOULD be treated as a URI Template per<<rfc6570,[RFC6570]>>. The follwing example shows how an UBER message snippet is converted into a valid HTTP query string:
...
Any +model+ value associated with a +data+ element SHOULD be convereted into a valid HTTP request body using the rules contained in <<rfc6570,[RFC6570]>>. The follwing example shows how an UBER message snippet is converted into a valid HTTP request body:
...
== Supporting Profiles
The UBER media-type supports the use of semantic profiles via the `profile' link relation type<<rfc6906,[RFC6906]>> in order to provide additional information about the application-level semantics for the message body. The profile value is a list of one or more IRIs identifying specific constraints or conventions that apply to an UBER document. 
...
This version of the UBER specification supports three ways to communicate profile information:
 ...
 . Within the UBER message body as a +data+ element with a +rel+ property value of +"profile"+.
 
Depending on the protocol in use, one or more of these methods MAY be used in the same request. For example, in an HTTP exchange a sender MAY use an `Accept` or `Content-Type` header that contains a profile value _and_ a `Link` header that contains a profile value _and_ a message body that contains a profile value. When this occurs, the recipient SHOULD honor all the profile values received. 
...
----
 Accept: application/vnd.amundsen.uber+json;profile=http://example.org/profiles/vcard
 Content-Type: application/vnd.amundsen.uber+xml;profile=http://example.org/profiles/vcard http://schema.org/Person
----
...
Profile information can be supplied as one or more +data+ elements withing an UBER message body. Examples of valid profile elements within an UBER document are:
