...
Any +model+ value associated with a +data+ element SHOULD be converted into a valid HTTP request body using the rules contained in <<rfc6570,[RFC6570]>>. The follwing example shows how an UBER document snippet is converted into a valid HTTP request body:
