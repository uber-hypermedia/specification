...
If the +templated+ property of a +data+ element is set to +true+ the +url+ property SHOULD be treated as a URI Template per <<rfc6570,[RFC6570]>>. The following example shows how an UBER message snippet is converted into a valid HTTP query string:
